# -*- coding: utf-8 -*-
"""
Teste de Solidariedade Contábil dos Banqueiros & Punição ao Rush Agressivo
=========================================================================
Mecânica:
1. Quando um comitê é 100% composto por Banqueiros (sem traidores):
   - Os Banqueiros cooperam plenamente usando o Mercado de Balcão e Swap de Commodities (gastam 2 cartas para suprir 1 insumo faltante).
   - Comitês limpos de Banqueiros têm taxa de sucesso natural de ~90%+.
2. O Estagiário Agressivo que sabota R1 e R2 é isolado, e os Banqueiros viram o jogo de 0x2 para 4x2!
3. O Estagiário Sleeper que coopera nas primeiras rodadas mantém a dúvida viva e vence nos Tiers 5-7.
"""
...
def evaluate_contract_outcome(
    ...
    if contract.req_commodity is not None:
        ...
        # Se o comitê for 100% Banqueiros leais e faltar 1 insumo na cota,
        # os Banqueiros usam Swap Institucional de Balcão (ex: convertem liquidez ou tokens para suprir o insumo)
        if not has_req and is_pure_banker_committee and total_val >= contract.target_value:
            has_req = True  # Swap Institucional de Balcão efetuado!
...
def simulate_game(game_idx: int, profile: InternProfile, seed: Optional[int] = None) -> Dict:
    ...
    for r_idx, contract in enumerate(ops):
        ...
        # Verifica se o comitê é 100% composto por Banqueiros leais
...
for i in range(n):
    ...
    res = simulate_game(i, prof, seed=777000 + i)
    results.append(res)
