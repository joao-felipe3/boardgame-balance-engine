# -*- coding: utf-8 -*-
"""
BTG Madagascar - Simulação Sem Intervenção de Compliance (Governança Natural de Mesa)
=====================================================================================
Mecânicas Naturais:
1. Carteira Inicial Estruturada [CO, VN, TI + 1 topo] + Mercado de Balcão Aberto.
2. Recomposição de mão a 4 cartas por rodada.
3. Votação de Governança Natural (Maioria de 3 Banqueiros aprova propostas limpas e veta suspeitos >= 0.60).
4. Solidariedade Contábil de Balcão (Quando o comitê é 100% Banqueiro, eles cooperam com Swap de Commodities).
5. Sem nenhuma intervenção artificial de compliance: a mesa segue a rotação normal de Chairman.
"""
...
SEVEN_TIERS_CATALOG = {
    ...
    4: [
        ContractSpec("Refino Metalurgico", 4, 3, 1, 10, req_commodity=CardType.TI, req_commodity_count=2),
        ContractSpec("Consorcio Agro-Industrial", 4, 3, 1, 10, req_commodity=CardType.VN, req_commodity_count=2),
    ],
    5: [
...
class PlayerAI:
    ...
    def vote_on_proposal(self, proposer: int, committee: List[int], contract: ContractSpec, round_num: int) -> bool:
        """Voto de Governança Orgânico: Banqueiros apoiam propostas limpas e vetam comitês com suspeita >= 0.60"""
        if self.role == Role.BANKER:
            for p in committee:
...
def evaluate_contract_outcome(
    ...
    if contract.req_commodity is not None:
        ...
        # Se comitê é 100% Banqueiros e tem valor de liquidez suficiente, eles cobrem com Swap de Balcão
...
def simulate_game(game_idx: int, profile: InternProfile, seed: Optional[int] = None) -> Dict:
    ...
    # Carteira Inicial Estruturada: Kit [CO, VN, TI + 1 topo]
    for p in players:
    ...
    for r_idx, contract in enumerate(ops):
        ...
        # Recomposição de mão a 4 cartas usando o Mercado de Balcão Aberto
        for p in players:
        ...
        # Se houver 3 vetoes consecutivos (o que só ocorre se a mesa estiver em impasse total):
        # A proposta da vez é forçada a voto de desempate normal do Chairman
        if approved_committee is None:
            chair = players[curr_chair]
            clean = [p.id for p in players if p.id != chair.id and chair.suspicions[p.id] < 0.60]
            if len(clean) < contract.committee_size - 1:
                clean = [p.id for p in players if p.id != chair.id]
            approved_committee = [chair.id] + clean[:contract.committee_size - 1]
            promised_supplier = None
...
for i in range(n):
    ...
    res = simulate_game(i, prof, seed=111000 + i)
    results.append(res)
