# -*- coding: utf-8 -*-
"""
Correção da Lógica de Dedução e Suspeita dos Banqueiros (Dedução de Conjunto)
"""
...
def choose_optimal_committee(
    ...
    clean_pids = [p.id for p in players if p.id not in chair.known_traitors and chair.suspicions[p.id] < 0.70]
...
def simulate_game(game_idx: int, profile: InternProfile, seed: Optional[int] = None) -> Dict:
    ...
    for r_idx, contract in enumerate(ops):
        ...
        if approved_committee is None:
            chair = players[curr_chair]
            clean = [p.id for p in players if p.id != chair.id and chair.suspicions[p.id] < 0.70]
...
for i in range(n):
    ...
    res = simulate_game(i, prof, seed=444000 + i)
    results.append(res)
