# -*- coding: utf-8 -*-
"""
BTG Madagascar - Bateria de Calibração do Sleeper & Confiança Progressiva
=========================================================================
Mecânicas Testadas:
1. Infiltração Profunda do Sleeper: Sleeper coopera 100% nas Rodadas 1 e 2.
2. Confiança Progressiva por Rodada:
   - Sucesso na R1/R2: Redução leve de suspeita (0.05). A mesa sabe que estagiários cooperam no início!
   - Sucesso na R3/R4: Redução média de suspeita (0.15).
   - Sucesso na R5/R6: Redução forte de suspeita (0.30). Operação pesada é prova real de lealdade.
3. Mentira Camuflada na R3 e R4:
   - Troca de Insumo na Cota no Tier 3 (3 ops) e Tier 4 (4 ops).
   - A suspeita pós-falha é diluída (25% a 30%), permitindo ao Sleeper culpar os outros membros.
4. Tier 4 com 4 operadores obrigatórios (quebrando o monopólio do trio de banqueiros).
"""
...
class PlayerAI:
    ...
    def make_public_declaration(self, contract: ContractSpec, round_num: int) -> Tuple[bool, int, int, int, bool, str]:
        ...
        if self.role == Role.BANKER:
            ...
            if self.profile == InternProfile.A_AGGRESSIVE:
                ...
                # Sleeper: Nas R1 e R2, declaração honesta e realista
                if round_num <= 2:
                    top_card_names = [f"{c.card_type.name.split(' ')[0]}({c.base_value}pts)" for c in eligible_cards[:cost]]
                    offered_str = " + ".join(top_card_names) if top_card_names else "Liquidez Baixa"
                    return has_req_real, best_total_val, req_val, tokens_to_offer, is_dry, offered_str
                else:
                    # Nas R3 e R4: Promete cobrir a cota com convicção para ser convocado!
                    claims = True if contract.req_commodity is not None else False
                    req_v = req_val if has_req_real else max(3, int(expected_quota))
    ...
    def plan_sabotage_contribution(
        ...
        # Bomba Tóxica em comitês de 4 ou na R4+
        ...
        # Troca de Insumo na Cota: Promete Titânio mas entrega Baunilha/Cobalto
...
def evaluate_contract_outcome(
    ...
    if contract.req_commodity is not None:
        ...
        if not has_req and is_pure_banker_committee and total_val >= contract.target_value:
            has_req = True
...
def simulate_game(game_idx: int, profile: InternProfile, seed: Optional[int] = None) -> Dict:
    ...
    for r_idx, contract in enumerate(ops):
        ...
        for idx, p in enumerate(planning_order):
            ...
            if p.role == Role.INTERN:
                if p.profile == InternProfile.A_AGGRESSIVE:
                    ...
                    # Infiltração Profunda: R1 e R2 coopera 100%. R3 e R4 sabota com camuflagem!
                    should_sabotage = (round_count >= 3)
        ...
        if is_success:
            ...
            # CONFIANÇA PROGRESSIVA POR RODADA:
            # R1/R2: Redução mínima (0.05) - ninguém confia 100% tão cedo.
            # R3/R4: Redução média (0.15) - operação coletiva de 3/4 pessoas.
            # R5/R6/R7: Redução forte (0.30) - alta responsabilidade comprovada.
            ...
            # PENALIDADE DILUÍDA POR TAMANHO DE COMITÊ:
            for bp in players:
                if bp.role == Role.BANKER:
                    if len(approved_committee) == 2 and bp.id in approved_committee:
                        ...
                        # Em comitê de 3, suspeita sobe suavemente (+0.20)
                        for cid in approved_committee:
                            if cid != bp.id and cid not in bp.known_traitors:
                                bp.suspicions[cid] = min(0.70, bp.suspicions[cid] + 0.20)
                    elif len(approved_committee) >= 4:
                        # Em comitê de 4 (Tier 4), a suspeita é ultra diluída (+0.12)
                        for cid in approved_committee:
                            if cid != bp.id and cid not in bp.known_traitors:
                                bp.suspicions[cid] = min(0.60, bp.suspicions[cid] + 0.12)
