# -*- coding: utf-8 -*-
"""
Ajuste Fino do Sleeper: Timing Tático de Sabotagem Diluída (R3/R4/R5)
=====================================================================
1. Agressivo: Sabota na R1 e R2 -> É exposto -> Banqueiros viram para 4x2 (WR Agressivo ~15-20%).
2. Sleeper: Joga limpo na R1 e R2 -> Ganha confiança -> Sabota na R3 (Troca de Insumo) e R4 (Bomba Tóxica Diluída) -> Leva a partida para o Clímax da 7ª Rodada (WR Sleeper ~45-50%).
"""
...
SEVEN_TIERS_CATALOG = {
    ...
    5: [
        ContractSpec("Fundicao Estrategica", 5, 3, 1, 10, req_commodity=CardType.TI, req_commodity_count=2),
        ContractSpec("Cofre de Gemas", 5, 3, 1, 11, req_commodity=CardType.SF, req_commodity_count=2),
    ],
    6: [
...
class PlayerAI:
    ...
    def plan_sabotage_contribution(
        ...
        if toxic_cards and (contract.committee_size >= 3 or round_num >= 4):
...
def simulate_game(game_idx: int, profile: InternProfile, seed: Optional[int] = None) -> Dict:
    ...
    for r_idx, contract in enumerate(ops):
        ...
        for idx, p in enumerate(planning_order):
            ...
            if p.role == Role.INTERN:
                if p.profile == InternProfile.A_AGGRESSIVE:
                    ...
                    # Sleeper: coopera na R1 e R2 para ganhar reputação impecável.
                    # Ataca a partir da R3 (comitê de 3 pessoas) diluindo a suspeita!
        ...
        if is_success:
            ...
            decay = 0.10 if contract.tier <= 2 else (0.18 if contract.tier <= 4 else 0.25)
            for bp in players:
            ...
            for bp in players:
                if bp.role == Role.BANKER:
                    if len(approved_committee) == 2 and bp.id in approved_committee:
                        ...
                        penalty = 0.20 if len(approved_committee) >= 4 else 0.30
                        for cid in approved_committee:
                            if cid != bp.id:
...
for i in range(n):
    ...
    res = simulate_game(i, prof, seed=555111 + i)
    results.append(res)
