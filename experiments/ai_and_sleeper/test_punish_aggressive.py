# -*- coding: utf-8 -*-
"""
Teste de Punição Severa ao Modo Agressivo & Governança Unificada dos Banqueiros
==============================================================================
Mecânicas:
1. Carteira Inicial Estruturada [CO, VN, TI + 1 topo] + Mercado de Balcão.
2. Governança em Bloco: Banqueiros com suspeita >= 0.60 são vetados com 3 votos NÃO.
3. Abastecimento Estratégico de Mercado: Banqueiros compram insumos necessários das cartas abertas.
4. Quando os 3 Banqueiros jogam juntos, o comitê opera com eficiência máxima e Ouro Coringa.
"""
...
class PlayerAI:
    ...
    def vote_on_proposal(self, proposer: int, committee: List[int], contract: ContractSpec, round_num: int) -> bool:
        """Governança Firme: Banqueiros vetam com rigor qualquer proposta com traidor conhecido ou suspeito >= 0.60"""
        if self.role == Role.BANKER:
            for p in committee:
    ...
    for r_idx, contract in enumerate(ops):
        ...
        for p in players:
            ...
            for _ in range(needed):
                # Se for Banqueiro e tiver a commodity da missão aberta no mercado, pega com prioridade!
                if p.role == Role.BANKER and contract.req_commodity in deck.open_market:
                    ...
                    # Pega Safira ou Titânio se tiver aberto
        ...
        if approved_committee is None:
            # Se vetou 3 vezes, o Chairman é forçado a escolher os mais limpos
            chair = players[curr_chair]
            ...
            for bp in players:
                if bp.role == Role.BANKER:
                    if len(approved_committee) == 2 and bp.id in approved_committee:
                        # Se comitê de 2 falhou e eu sou Banqueiro, o outro é 100% TRAIDOR!
...
for i in range(n):
    ...
    res = simulate_game(i, prof, seed=555000 + i)
    results.append(res)
