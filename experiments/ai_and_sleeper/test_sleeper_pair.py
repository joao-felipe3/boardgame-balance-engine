# -*- coding: utf-8 -*-
"""
Estratégia de Dupla de Infiltração (Agente Ativo + Agente Reserva)
=================================================================
Mecânica:
- Estagiário 1 (Ativo): Se infiltra e sabota na R3 (dividindo a culpa no comitê de 3).
- Estagiário 2 (Reserva): Mantém ficha limpíssima no banco. Quando o Estagiário 1 é suspeito na R3, os Banqueiros convocam o Estagiário 2 na R4/R5 achando que ele é o Banqueiro que estava no banco!
- O Estagiário 2 desfere o golpe fatal na R5/R6 e vence no Clímax da 7ª Rodada!
- Agressivo (ambos sabotam na R1/R2): Ambos são queimados e perdem 4x2 para os Banqueiros.
"""
...
def simulate_game(game_idx: int, profile: InternProfile, seed: Optional[int] = None) -> Dict:
    ...
    # Designa 1 Estagiário como Agente Ativo e 1 como Agente Reserva
    intern_count = 0
    players = []
    for i in range(5):
        is_active = False
            ...
            if p.role == Role.INTERN:
                if p.profile == InternProfile.A_AGGRESSIVE:
                    ...
                    if p.is_active_saboteur:
                        # Estagiário Ativo: coopera R1/R2, sabota na R3/R4
                        should_sabotage = (round_count >= 3)
                    else:
                        # Estagiário Reserva: coopera R1/R2/R3, sabota na R4/R5/R6 quando é convocado!
...
for i in range(n):
    ...
    res = simulate_game(i, prof, seed=999111 + i)
    results.append(res)
