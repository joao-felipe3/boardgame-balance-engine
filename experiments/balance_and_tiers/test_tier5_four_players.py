# -*- coding: utf-8 -*-
"""
BTG Madagascar - Simulação com Comitê de 4 Operadores no Tier 5 (R5)
====================================================================
Mecânicas Testadas:
1. Comitê de 4 jogadores no Tier 5 (R5): Os Banqueiros acumularam tokens (+1🪙) nas R1-R4 e usam para combater o Ativo Tóxico (-4).
2. Estratégia de Dupla Sleeper (Agente Ativo + Agente Reserva).
3. Confiança Progressiva por Rodada:
   - R1/R2: Redução leve (0.05).
   - R3/R4: Redução média (0.15).
   - R5+: Redução forte (0.30).
4. Catálogo dos 7 Tiers:
   - T1: 2 ops (meta 3-4)
   - T2: 2 ops (meta 4)
   - T3: 3 ops (meta 8, req 2 TI)
   - T4: 3 ops (meta 10, req 2 VN/TI)
   - T5: 4 ops (meta 14, req 2 TI) [🔥 O Megaconsórcio com 4 pessoas na R5!]
   - T6: 3 ops (meta 16, req 2 SF, 2 cartas/op)
   - T7: 3 ops (meta 20, req 2 SF, 2 cartas/op)
"""
...
SEVEN_TIERS_CATALOG = {
    ...
    5: [
        ContractSpec("Megaconsorcio Industrial", 5, 4, 1, 14, req_commodity=CardType.TI, req_commodity_count=2),
        ContractSpec("Cofre de Commodities", 5, 4, 1, 14, req_commodity=CardType.VN, req_commodity_count=2),
    ],
    6: [
    ...
    7: [
        ContractSpec("Holding Global BTG (Climax)", 7, 3, 2, 20, req_commodity=CardType.SF, req_commodity_count=2),
    ]
}
...
class PlayerAI:
    ...
    def plan_honest_contribution(
        ...
        if is_match_point or contract.tier >= 6 or contract.committee_size >= 4:
            # Em comitês grandes de 4 ou na reta final, os Banqueiros usam todos os tokens necessários para cobrir Ativos Tóxicos!
...
for i in range(n):
    ...
    res = simulate_game(i, prof, seed=888333 + i)
    results.append(res)
