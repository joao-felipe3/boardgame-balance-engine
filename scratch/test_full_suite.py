...
# 1. Update choose_optimal_committee
orig_choose = engine.choose_optimal_committee

def smart_choose(chair, players, contract, declarations, conflict_pairs=None, quarantined_pid=None, public_accused=None, passed_members=None):
    ...
    if chair.role == Role.BANKER and accused_set:
        ...
        clean_pids = unaccused_clean if len(unaccused_clean) >= c_size else [
            p.id for p in valid_players if p.id not in chair.known_traitors and chair.suspicions[p.id] < 0.60
        ]
    else:
        clean_pids = [
            p.id for p in valid_players if p.id not in chair.known_traitors and chair.suspicions[p.id] < 0.60
...
engine.choose_optimal_committee = smart_choose

# 2. Update vote_on_proposal
orig_vote = player_mod.PlayerAI.vote_on_proposal

def smart_vote(self, proposer, committee, contract, round_num, conflict_pairs=None, consecutive_vetoes=0, declarations=None):
    if self.role == Role.BANKER:
        ...
        # Conversa pré-comitê: insumos e metas garantidos
        if declarations:
            if contract.req_commodity is not None:
                suppliers = [pid for pid in committee if declarations[pid].claims_req]
                if len(suppliers) < contract.req_commodity_count:
                    return False
            total_dec = sum(declarations[pid].claimed_value for pid in committee)
            if total_dec < contract.target_value:
                return False

        # Isolamento de suspeito: nunca aprovar 2 suspeitos num comitê de 3+
        if len(committee) >= 3:
            if self.id in committee:
                others = [p for p in committee if p != self.id]
                suspects = sum(1 for p in others if self.suspicions[p] >= 0.40)
                if suspects >= 2:
                    return False
            else:
                suspects = sum(1 for p in committee if self.suspicions[p] >= 0.40)
                if suspects >= 2:
                    return False
...
player_mod.PlayerAI.vote_on_proposal = smart_vote
...
print(f"Results with smart_choose + smart_vote:")
